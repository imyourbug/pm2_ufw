...
def neurons_to_ips(all_neurons: Dict[int, List["bt.NeuronInfoLite"]]) -> Set[str]:
    ...
    for _, subnet_neurons in all_neurons.items():
        ips = [
            neuron.axon_info.ip for neuron in subnet_neurons if neuron.validator_permit
        ]
...
def whitelist_ips_in_ufw(ips: List[str]):
    ...
    # Deny all IP
    ssh_cmd = "sudo ufw deny in from any"
    subprocess.run(ssh_cmd, shell=True)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--netuid",
        help="Subnet(s) to add firewall to",
        choices=[6, 15, 32, 34, 43],
        type=int,
        nargs="+",
    )
    parser.add_argument(
